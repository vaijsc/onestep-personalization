...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--pretrained_arcface_path",
        type=str,
        default="../all_backup_SP/ckpt/face/model_ir_se50.pth",
        required=False,
...
def main(args):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                for i, model in enumerate(models):
                    if isinstance(model, type(accelerator.unwrap_model(unet_with_ipa))):
    ...
    # define loss module
    loss_identity_calculator = IDLoss(args.pretrained_arcface_path, accelerator=accelerator, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet_with_ipa):
                ...
                out_image = (
                    vae.decode(out_image.to(dtype=weight_dtype)).sample.float()
                )
                
                loss_identity = loss_identity_calculator(out_image, batch["ref_pixel_values"])
                ...
                accelerator.backward(loss_identity)
