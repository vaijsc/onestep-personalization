...
def inference(vae, tokenizer, text_encoder,
    ...
    # noise = gen_random_tensor_fix_seed((1, 4, 64, 64), 0, weight_dtype, device)
    ...
    noise_pred = unet_with_ipa(noise, final_timestep, prompt_embeds, clip_image_embeds)
...
def main(args):
    ...
    os_unet = UNet2DConditionModel.from_pretrained(
    ...
    unet_with_ipa = get_ipa_generator(os_unet, image_encoder, "teacher", attn_processor="nest")
    ...
    # define loss module
    loss_dino_calculator = DiNOLoss(accelerator.device)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet_with_ipa):
                ...
                # decode to clean image and then feed to identity network
                
                out_image = pred_img_latents / vae.config.scaling_factor
                out_image = (
                    vae.decode(out_image.to(dtype=weight_dtype)).sample.float() + 1
                ) / 2
                
                # Identity semantic matching loss
                loss_identity = loss_dino_calculator(ref_image, out_image)
                avg_loss_identity = accelerator.gather(loss_identity.repeat(args.train_batch_size)).mean()
                train_loss_identity += avg_loss_identity.item() / args.gradient_accumulation_steps

                # L2 regression loss
                gt_x = (batch["out_pixel_values"] + 1) / 2
                loss_l2x = F.mse_loss(gt_x.float(), out_image.float(), reduction="mean")
                avg_loss_l2x = accelerator.gather(loss_l2x.repeat(args.train_batch_size)).mean()
                train_loss_l2x += avg_loss_l2x.item() / args.gradient_accumulation_steps

                accelerator.backward(loss_identity + loss_l2x)
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log(
                    {
                        "train_loss_identity": train_loss_identity,
                        "train_loss_l2x": train_loss_l2x,
                    }, step=global_step
                )
            ...
            logs = {
                "step_loss_identity": (loss_identity).detach().item(),
                "step_loss_l2x": (loss_l2x).detach().item(),
