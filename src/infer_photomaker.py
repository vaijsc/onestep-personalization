...
def load_photomaker_adapter(
    ...
    **kwargs,
    ...
    print(
        f"Loading PhotoMaker {pm_version} components [1] id_encoder from [{pretrained_model_name_or_path_or_dict}]..."
    )
    ...
    print(
        f"Loading PhotoMaker {pm_version} components [2] lora_weights from [{pretrained_model_name_or_path_or_dict}]"
    )
    pipe.load_lora_weights(state_dict["lora_weights"], adapter_name="photomaker")

    # Add trigger word token
    if pipe.tokenizer is not None:
        pipe.tokenizer.add_tokens([trigger_word], special_tokens=True)

    pipe.tokenizer_2.add_tokens([trigger_word], special_tokens=True)

    return id_encoder, id_image_processor, trigger_word, num_tokens, pm_version
...
def encode_prompt_with_trigger_word(
    ...
    image_token_id = pipe.tokenizer_2.convert_tokens_to_ids(trigger_word)
    ...
    if prompt_embeds is None:
        ...
        for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
            ...
            clean_input_ids = (
                ...
                + [class_token] * num_id_images * num_tokens
            ...
            class_tokens_mask = [
                True if class_token_index <= i < class_token_index + (num_id_images * num_tokens) else False
    ...
    # get unconditional embeddings for classifier free guidance
    zero_out_negative_prompt = negative_prompt is None  # and config.force_zeros_for_empty_prompt
    if do_classifier_free_guidance and negative_prompt_embeds is None and zero_out_negative_prompt:
        negative_prompt_embeds = torch.zeros_like(prompt_embeds)
        negative_pooled_prompt_embeds = torch.zeros_like(pooled_prompt_embeds)
    elif do_classifier_free_guidance and negative_prompt_embeds is None:
...
def infer(
    ...
    pipe: StableDiffusionXLPipeline,
    id_encoder: Union[PhotoMakerIDEncoder, PhotoMakerIDEncoder_CLIPInsightfaceExtendtoken],
    id_image_processor: CLIPImageProcessor,
    trigger_word: str,
    ...
    **kwargs,
    ...
    (
        ...
        _,
        class_tokens_mask,
    ) = encode_prompt_with_trigger_word(
        prompt=prompt,
        pipe=pipe,
        trigger_word=trigger_word,
    ...
    tokens_text_only = pipe.tokenizer.encode(prompt, add_special_tokens=False)
    trigger_word_token = pipe.tokenizer.convert_tokens_to_ids(trigger_word)
    tokens_text_only.remove(trigger_word_token)
    prompt_text_only = pipe.tokenizer.decode(tokens_text_only, add_special_tokens=False)
    (
    ...
    # 5. Prepare timesteps
    timesteps, num_inference_steps = retrieve_timesteps(pipe.scheduler, num_inference_steps, device, timesteps, sigmas)
    ...
    if id_embeds is not None:
        ...
        prompt_embeds = id_encoder(id_pixel_values, prompt_embeds, class_tokens_mask, id_embeds)
    else:
    ...
    if denoising_end is not None and isinstance(denoising_end, float) and denoising_end > 0 and denoising_end < 1:
        discrete_timestep_cutoff = int(
            round(
                pipe.scheduler.config.num_train_timesteps - (denoising_end * pipe.scheduler.config.num_train_timesteps)
            )
        )
    ...
    with pipe.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            if i <= start_merge_step:
                ...
                add_text_embeds = (
                    torch.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds_text_only], dim=0)
                    if do_classifier_free_guidance
                    else pooled_prompt_embeds_text_only
                )
            else:
                current_prompt_embeds = (
...
pipe = StableDiffusionXLPipeline.from_pretrained("SG161222/RealVisXL_V4.0", torch_dtype=torch.float16).to("cuda")
id_encoder, id_image_processor, trigger_word, num_tokens, pm_version = load_photomaker_adapter(
...
core_prompt = "photo of a woman img"
prompt = f"instagram photo, portrait {core_prompt}, colorful, perfect face, natural skin, hard shadows, film grain, best quality"
negative_prompt = (
...
images = infer(
    ...
    id_image_processor=id_image_processor,
    trigger_word=trigger_word,
    negative_prompt=negative_prompt,
    input_id_images=input_id_images,
    id_embeds=id_embeds,
...
for idx, img in enumerate(images):
    img.save(os.path.join(output_dir, f"{core_prompt}_{idx}.jpg"))
