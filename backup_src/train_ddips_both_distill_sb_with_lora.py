...
def get_x0_from_noise(sample, model_output, noise_scheduler):
...
def inference(vae, tokenizer, text_encoder,
              noise_scheduler, student_generator_lora_with_ipa, 
              teacher_with_ipa,
              image_encoder,
    ...
    student_generator_lora_with_ipa = accelerator.unwrap_model(student_generator_lora_with_ipa)
    ...
    # noise = torch.randn(input_shape, dtype=weight_dtype, device=accelerator.device)
    noise = gen_random_tensor_fix_seed(input_shape, 0, weight_dtype, accelerator.device)
    ...
    noise_pred = student_generator_lora_with_ipa(noise, final_timestep, prompt_embeds, image_embeds)
    pred_latents = get_x0_from_noise(
        noise, noise_pred, noise_scheduler
    )
    ...
    # Get denoised image with one step image generator (sbv2 in this case) with ip-condition (scale=0)
    student_generator_lora_with_ipa.set_scale(0)
    noise_pred = student_generator_lora_with_ipa(noise, final_timestep, prompt_embeds, image_embeds)
    pred_latents = get_x0_from_noise(
        noise, noise_pred, noise_scheduler
    )
    ...
    # set scale back to 1
    student_generator_lora_with_ipa.set_scale(1)

    ## Get prediction of teacher as well for comparison
    noise_scheduler.set_timesteps(NUM_STEPS, device=accelerator.device)
    timesteps = noise_scheduler.timesteps

    null_prompt_embeds = encode_prompt([""], text_encoder, tokenizer)["prompt_embeds"].to(device="cuda")
    null_image_embeds = torch.zeros_like(image_embeds).to(accelerator.device)
    latents = noise
    ...
    for i, t in enumerate(tqdm(timesteps)):
        ...
        noise_pred_uncond = teacher_with_ipa(latent_model_input, t, null_prompt_embeds, null_image_embeds)
        noise_pred = teacher_with_ipa(latent_model_input, t, prompt_embeds, image_embeds)

        noise_pred = noise_pred_uncond + CFG * (noise_pred - noise_pred_uncond) 
        latents = noise_scheduler.step(noise_pred, t, latents, return_dict=False)[0]

    out_image_teacher = latents / vae.config.scaling_factor
    out_image_teacher = (
        vae.decode(out_image_teacher.to(dtype=weight_dtype)).sample.float() + 1
    ) / 2
    
    return out_image.detach().cpu()[0], out_image_scale0.detach().cpu()[0], out_image_teacher.detach().cpu()[0]
...
def main(args):
    ...
    student_unet_gen_ip = UNet2DConditionModel.from_pretrained(
        args.pretrained_sb_generator, 
        subfolder="unet_ema"
    ).to("cuda")

    student_unet_gen_ip.to(accelerator.device, dtype=weight_dtype).eval()
    # student_unet_gen_ip.requires_grad_(False)
    ...
    student_generator_lora_with_ipa = get_ipa_generator(student_unet_gen_ip, image_encoder, "student", args.num_ip_tokens)    
    teacher_with_ipa = get_ipa_generator(teacher_unet, image_encoder, "teacher")
    
    if args.pretrained_ip_adapter is not None:
        student_generator_lora_with_ipa.load_from_checkpoint(args.pretrained_ip_adapter)
        teacher_with_ipa.load_from_checkpoint(args.pretrained_ip_adapter)
    ...
    student_generator_lora_with_ipa.unet.add_adapter(config)
    student_generator_lora_with_ipa.unet.train()
    
    # manually set to_k_ip and to_v_ip trainable due to some issue
    for name, param in student_generator_lora_with_ipa.unet.named_parameters():
    ...
    # Optimizer creation
    params = [
            student_generator_lora_with_ipa.image_proj_model.parameters(),
            student_generator_lora_with_ipa.adapter_modules.parameters(),
            student_generator_lora_with_ipa.unet.parameters()] ## adapter of ip and lora training
    
    # params = [
    #         student_generator_lora_with_ipa.image_proj_model.parameters(),
    #         student_generator_lora_with_ipa.adapter_modules.parameters()] ## adapter of ip and lora training
    ...
    ## check trainable parameteres here
    # for name, param in student_generator_lora_with_ipa.unet.named_parameters():
    #     if "to_k_ip" in name or "to_v_ip" in name:
    ...
    ref_image_column, text_column = column_names
    ...
    student_generator_lora_with_ipa, optimizer, teacher_with_ipa, train_dataloader, lr_scheduler = (
        accelerator.prepare(
            student_generator_lora_with_ipa, optimizer, teacher_with_ipa, train_dataloader, lr_scheduler
        )
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(student_generator_lora_with_ipa):

                bsz = batch["ref_pixel_values"].shape[0]
                # Sampling noise
                noise = torch.randn((bsz, 4, 64, 64)).to(accelerator.device)
                # noise = gen_random_tensor_fix_seed((bsz, 4, 64, 64), 0, weight_dtype, accelerator.device)
                ...
                null_image_embeds = torch.zeros_like(clip_image_embeds).to(accelerator.device)

                # Get denoised image with one step image generator (sbv2-sd1.5 in this case)
                noise_pred = student_generator_lora_with_ipa(noise, final_timestep, prompt_embeds, clip_image_embeds)

                # denoising loss for lora unet
                # ## Denoising Loss
                ...
                pred_img_latents = get_x0_from_noise(
                    noise, noise_pred, noise_scheduler
                )
                ...
                # (this is the teacher forward diffusion process)

                noise_add = torch.randn_like(noise)
                noisy_samples = noise_scheduler.add_noise(pred_img_latents, noise_add, timesteps)

                # Prepare outputs from the teacher
                with torch.no_grad():
                    teacher_pred_uncond_img_uncond_text = teacher_with_ipa(noisy_samples, timesteps, prompt_null_embeds, null_image_embeds)
                    teacher_pred_cond_img_uncond_text = teacher_with_ipa(noisy_samples, timesteps, prompt_null_embeds, clip_image_embeds)
                    teacher_pred_uncond_img_cond_text = teacher_with_ipa(noisy_samples, timesteps, prompt_embeds, null_image_embeds)
                    
                    score = teacher_pred_uncond_img_uncond_text + args.guidance_scale * (
                        teacher_pred_cond_img_uncond_text - teacher_pred_uncond_img_uncond_text
                    ) + args.guidance_scale * (
                        teacher_pred_uncond_img_cond_text - teacher_pred_uncond_img_uncond_text
                    )

                sigma_t = ((1 - alphas_cumprod[timesteps]) ** 0.5).view(-1, 1, 1, 1)
                score_gradient = torch.nan_to_num(sigma_t**2 * (score - noise_add))

                target = (pred_img_latents - score_gradient).detach()

                # Compute loss
                loss_ddip = 0.5 * F.mse_loss(pred_img_latents.float(), target.float(), reduction="mean")
                loss_ddip *= 1000

                avg_loss_ddip = accelerator.gather(loss_ddip.repeat(args.train_batch_size)).mean()
                train_loss_ddip += avg_loss_ddip.item() / args.gradient_accumulation_steps

                accelerator.backward(loss_ddip)
                
                # for name, param in student_generator_lora_with_ipa.module.named_parameters():
                #     if param.grad is not None:
                #         print(f"Gradient for {name}: {param.grad.norm()}")  # Print the norm of the gradient
                #     else:
                #         print(f"No gradient for {name}")
                
                # if accelerator.sync_gradients:
                #     accelerator.clip_grad_norm_(student_generator_lora_with_ipa.image_proj_model.parameters(), args.max_grad_norm)
                #     accelerator.clip_grad_norm_(student_generator_lora_with_ipa.adapter_modules.parameters(), args.max_grad_norm)
                #     accelerator.clip_grad_norm_(student_generator_lora_with_ipa.unet.parameters(), args.max_grad_norm)
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log(
                    {
                        "train_loss_ddip": train_loss_ddip,
                ...
                if global_step % args.validation_steps == 0 \
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        out_image_teacher_stores = {}
                        
                        # on train set
                        log_image_test = {
                            "train": train_dict_data,
                            "val": val_dict_data}
                        ...
                        for split in log_image_test:
                            ...
                            for idx in range(len_samples):
                                ...
                                out_image, out_image_scale0, out_image_teacher = inference(
                                    vae,
                                    tokenizer,
                                    text_encoder,
                                    noise_scheduler,
                                    student_generator_lora_with_ipa,
                                    teacher_with_ipa,
                                    image_encoder,
                                    text,
                                    inp_img_path,
                                ...
                                out_image_teacher_stores[key_reconstruct_image] = out_image_teacher
                        ...
                        for split in log_image_test:
                            ...
                            for idx in range(len_samples):
                                ...
                                vis_imgs = torch.cat([ref_img,
                                                    ...
                                                    out_image_stores_scale0[key_reconstruct_image],
                                                    out_image_teacher_stores[key_reconstruct_image]], dim=2)
            ...
            logs = {
                "step_loss_ddip": (loss_ddip).detach().item(),
