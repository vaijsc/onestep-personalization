...
def main(args):
            ...
            for i in range(len(models)):
                ...
                if isinstance(model, type(accelerator.unwrap_model(sb_ip_adapter_generator))):
                    ...
                    model = UNet2DConditionModel.from_pretrained(
                        input_dir, subfolder="teacher_lora"
                    )
    ...
    # Optimizer creation
    params = [
            sb_ip_adapter_generator.image_proj_model.parameters(),
    ...
    sb_ip_adapter_generator, optimizer, teacher_lora, optimizer_lora, train_dataloader, lr_scheduler, lr_scheduler_lora = (
        accelerator.prepare(
            sb_ip_adapter_generator, optimizer, teacher_lora, optimizer_lora, train_dataloader, lr_scheduler, lr_scheduler_lora
        )
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        train_loss_elpips = 0.0
        train_loss_vsd = 0.0
        train_loss_lora = 0.0
        
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(sb_ip_adapter_generator, teacher_lora):
                # Get GT image latent
                with torch.no_grad():
                ...
                noise_pred = sb_ip_adapter_generator(noisy_latents, last_timestep, prompt_embeds, image_embeds)
                pred_img_latents = (noisy_latents - sigma_t * noise_pred) / alpha_t
                ...
                ## ELPIPS Loss                
                loss_elpips = elatentlpips(pred_img_latents.float(), gt_latents.float(), normalize=False).mean()
                avg_loss_elpips = accelerator.gather(loss_elpips.repeat(args.train_batch_size)).mean()
                train_loss_elpips += avg_loss_elpips.item() / args.gradient_accumulation_steps
                
                # Backpropagate
                accelerator.backward(loss_elpips + loss_vsd)
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log(
                    {
                        "train_loss_elpips": train_loss_elpips,
            ...
            logs = {
                "step_loss_elpips": (loss_elpips).detach().item(),
