...
def get_noisy_latent_from_x0(noise, x0, timestep):
    alpha_prod_t = (torch.ones_like(timestep).float() * 0.0047).reshape(-1, 1, 1, 1) 
    beta_prod_t = 1 - alpha_prod_t

    noisy_latents = x0 * (alpha_prod_t ** (0.5)) + noise * (beta_prod_t ** (0.5))
...
def inference(vae, tokenizer, text_encoder,
    ...
    one_timestep = torch.ones(1, device=device, dtype=torch.long)
    ...
    noisy_latents = get_noisy_latent_from_x0(noise, latents, one_timestep).to(device="cuda")
    ...
    # Get denoised image with one step image generator (dmdv2 in this case) with ip-condition (scale=1)
    noise_pred = student_generator_with_ipa(noisy_latents, final_timestep, prompt_embeds, image_embeds)
    pred_latents = get_x0_from_noise(noisy_latents, noise_pred, one_timestep)
    ...
    # Get denoised image with one step image generator (dmdv2 in this case) with ip-condition (scale=0)
...
def main(args):
    ...
    logger.info(args.pretrained_dmd_generator)

    student_unet_gen_ip = create_generator(args.pretrained_dmd_generator)
    ...
    teacher_lora_with_ipa = get_ipa_generator(teacher_lora, image_encoder, "teacher")
    ...
    teacher_lora_with_ipa.unet.add_adapter(config) # lora adapter

    teacher_lora_with_ipa.unet.train()
    del teacher_lora

    if args.pretrained_ip_adapter is not None:
        student_generator_with_ipa.load_from_checkpoint(args.pretrained_ip_adapter)
        teacher_lora_with_ipa.load_from_checkpoint(args.pretrained_ip_adapter)
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                for i, model in enumerate(models):
                    ...
                    elif model.name == "teacher":
                        # subfolder = "teacher_lora"
                        # model.unet.save_pretrained(os.path.join(output_dir, subfolder))
                        torch.save(model.unet.state_dict(), os.path.join(output_dir, "unet_teacher.bin"))
        ...
        def load_model_hook(models, input_dir):
            for i in range(len(models)):
                ...
                if model.name == "student":
                    ...
                    model = teacher_lora_with_ipa
                    model.unet.load_state_dict(torch.load(os.path.join(input_dir, "unet_teacher.bin")))
    ...
    #     teacher_lora_with_ipa.enable_gradient_checkpointing()
    ...
    if args.use_8bit_adam:
        ...
        optimizer_lora_class = bnb.optim.Adam8Wbit
    else:
    ...
    params = [
            ...
            student_generator_with_ipa.adapter_modules.parameters()]
    ...
    optimizer_lora = optimizer_lora_class(
        teacher_lora_with_ipa.unet.parameters(),
    ...
    student_generator_with_ipa, optimizer, teacher_lora_with_ipa, optimizer_lora, train_dataloader, lr_scheduler, lr_scheduler_lora = (
        accelerator.prepare(
            student_generator_with_ipa, optimizer, teacher_lora_with_ipa, optimizer_lora, train_dataloader, lr_scheduler, lr_scheduler_lora
        )
    )
    ...
    # test train without ip-adapter in teacher (lora and freeze)
    teacher_lora_with_ipa.set_scale(0)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(student_generator_with_ipa, teacher_lora_with_ipa):
                # Get GT image latent
                with torch.no_grad():
                ...
                noisy_latents = get_noisy_latent_from_x0(noise, gt_latents, one_timestep)
                ...
                # Get denoised image with one step image generator (dmd2 in this case)
                noise_pred = student_generator_with_ipa(noisy_latents, final_timestep, prompt_embeds, image_embeds)
                pred_img_latents = get_x0_from_noise(
                    noisy_latents, noise_pred, one_timestep
                )
                ...
                noise = torch.randn_like(gt_latents)
                noisy_samples = noise_scheduler.add_noise(pred_img_latents, noise, timesteps)

                # Prepare outputs from the teacher
                with torch.no_grad():
                    accelerator.unwrap_model(teacher_lora_with_ipa.unet).disable_adapters()

                    teacher_pred_cond = teacher_lora_with_ipa(noisy_samples, timesteps, prompt_embeds, image_embeds)
                    teacher_pred_uncond = teacher_lora_with_ipa(noisy_samples, timesteps, prompt_null_embeds, image_embeds)

                    accelerator.unwrap_model(teacher_lora_with_ipa.unet).enable_adapters()
                    lora_pred_cond = teacher_lora_with_ipa(noisy_samples, timesteps, prompt_embeds, image_embeds)
                    lora_pred_uncond = teacher_lora_with_ipa(noisy_samples, timesteps, prompt_null_embeds, image_embeds)
                ...
                loss_vsd = 0.5 * F.mse_loss(pred_img_latents.float(), target.float(), reduction="mean")
                ...
                # Backpropagate
                # accelerator.backward(loss_diff + loss_vsd)
                ...
                noise = torch.randn_like(pred_img_latents.detach())
                ...
                noisy_samples = noise_scheduler.add_noise(pred_img_latents.detach(), noise, timesteps)
                ...
                lora_pred = teacher_lora_with_ipa(noisy_samples, timesteps, encoder_hidden_states, image_embeds)

                alpha_t = (alphas_cumprod[timesteps] ** 0.5).view(-1, 1, 1, 1)
                lora_pred, target = alpha_t * lora_pred, alpha_t * noise
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(teacher_lora_with_ipa.unet.parameters(), args.max_grad_norm)
                optimizer_lora.step()
