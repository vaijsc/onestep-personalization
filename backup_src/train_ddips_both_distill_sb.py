...
def inference(vae, tokenizer, text_encoder,
              noise_scheduler, student_generator_with_ipa, 
              teacher_with_ipa,
              image_encoder,
    ...
    student_generator_with_ipa = accelerator.unwrap_model(student_generator_with_ipa)
    ...
    noise_pred = student_generator_with_ipa(noise, final_timestep, prompt_embeds, image_embeds)
    ...
    student_generator_with_ipa.set_scale(0)
    noise_pred = student_generator_with_ipa(noise, final_timestep, prompt_embeds, image_embeds)
    pred_latents = get_x0_from_noise(
        noise, noise_pred, noise_scheduler
    )
    ...
    # set scale back to 1
    student_generator_with_ipa.set_scale(1)
...
def main(args):
    ...
    student_generator_with_ipa = get_ipa_generator(student_unet_gen_ip, image_encoder, "student", args.num_ip_tokens)    
    teacher_with_ipa = get_ipa_generator(teacher_unet, image_encoder, "teacher")
    
    if args.pretrained_ip_adapter is not None:
        student_generator_with_ipa.load_from_checkpoint(args.pretrained_ip_adapter)
    ...
    for name, param in student_generator_with_ipa.unet.named_parameters():
    ...
    #     student_generator_with_ipa.enable_gradient_checkpointing()
    ...
    # Optimizer creation
    params = [
            student_generator_with_ipa.image_proj_model.parameters(),
            student_generator_with_ipa.adapter_modules.parameters()] ## adapter of ip and lora training
    ...
    student_generator_with_ipa, optimizer, teacher_with_ipa, train_dataloader, lr_scheduler = (
        accelerator.prepare(
            student_generator_with_ipa, optimizer, teacher_with_ipa, train_dataloader, lr_scheduler
        )
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(student_generator_with_ipa):
                ...
                noise_pred = student_generator_with_ipa(noise, final_timestep, prompt_embeds, clip_image_embeds)
                ...
                # for name, param in student_generator_with_ipa.module.named_parameters():
                ...
                #     accelerator.clip_grad_norm_(student_generator_with_ipa.image_proj_model.parameters(), args.max_grad_norm)
                #     accelerator.clip_grad_norm_(student_generator_with_ipa.adapter_modules.parameters(), args.max_grad_norm)
                #     accelerator.clip_grad_norm_(student_generator_with_ipa.unet.parameters(), args.max_grad_norm)
